...
for($i=0; $i<count($this->featuredch); $i++) {
	...
	if($this->featuredch[$i]==NULL)
		...
		if($this->featuredch[$i]->custom_code == "") {
			...
			$query = 'SELECT id FROM #__menu WHERE title = "' . $this->featuredch[$i]->title . '"';
			$db->setQuery($query);
            ...
            echo "<tr>";
                echo "<td colspan='4'>";
			        echo "<h2 class=\"cattitle\"><a href=index.php?option=com_featuredch&view=featuredcontent&Itemid=" . $result . ">" . $this->featuredch[$i]->title . "</a></h2>";
                echo "</td>";
            echo "</tr>";
